...
This means Kubernetes is allowed to take a maximum of zero pods down while updating. Having this at 0 does slow the process down a bit, but it maintains consistent availability, which 1 wouldn't.
...
Utilization would likely rise above this target if requests doubled, so more pods would be created. 8 pods being the maximum, additional traffic at that point would increase latency or potentially lead to failure.
